...
export default function loadPosts(data: any) {
  ...
  posts.forEach(
    async (post: {
      ...
      const loadcomments = await loadComments(post.id);
      //console.log(loadcomments.data.comments[0]);
      //console.log(loadcomments.data.comments);
      //console.log(loadcomments.status);

      if (loadcomments.status === 200) {
        const comments = loadcomments.data.comments;
